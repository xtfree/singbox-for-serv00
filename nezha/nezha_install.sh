...
install_agent() {
  install -m 755 ${TMP_DIRECTORY}/nezha-agent ${NEZHA_DIR}/nezha-agent
}

generate_run_agent_start_sh() {
...
nohup $NEZHA_DIR/start.sh >/dev/null 2>&1 &
EOF
...
install_and_run_nezha_agent() {
  ...
  if [ ! -e "${NEZHA_DIR}/nezha-agent" ]; then
    download_agent
